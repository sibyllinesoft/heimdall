...
export class GBDTRuntime {
  ...
  async predict(
    ...
    try {
      ...
      // Clamp probabilities to [0, 1] range and normalize
      const clamped = [
        Math.max(0, Math.min(1, predictions[0])),
        Math.max(0, Math.min(1, predictions[1])),
        Math.max(0, Math.min(1, predictions[2]))
      ];
      
      // Normalize to ensure they sum to 1.0
      const total = clamped[0] + clamped[1] + clamped[2];
      const normalized = total > 0 ? clamped.map(p => p / total) : [1/3, 1/3, 1/3];
